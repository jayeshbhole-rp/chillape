    ...
    <CommandDialog open={open} onOpenChange={setOpen}>
      ...
      <CommandList className="max-h-[600px] pb-1">
        ...
        <CommandGroup heading="Ethereum">
          {evmConnectors.map((wallet, index) => (
            <CommandItem
              ...
              value={wallet.name}
              disabled={Boolean(
                (!getWallet(wallet.id, wallet) ||
                  getWallet(wallet.id, wallet)?.disabled) ??
        ...
        <CommandGroup heading="Tron">
          {WALLETS.filter((wallet) => wallet.type === NetworkType.TRON).map(
            (wallet, index) => (
              <CommandItem
                key={`${index} ${wallet.id}`}
                value={wallet.id}
        ...
        <CommandGroup heading="Cosmos">
          {WALLETS.filter((wallet) => wallet.type === NetworkType.COSMOS).map(
            (wallet, index) => (
              <CommandItem
                key={`${index} ${wallet.id}`}
                value={wallet.id}
