...
const AdapterNode = ({ adapterIndex }: { adapterIndex: string }) => {
  const { adapterStatuses: adapterStatuses, adapterMap } =
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="absolute right-4 top-4 opacity-5">
          {adapterData.adapterIndex}
        </span>{" "}
          ...
          <div className="flex flex-col gap-2">
            <div className="flex items-end justify-between">
              <span className="text-neutral-400">
                {isAdapterBridge(adapterData.adapterType) ? "Source" : "Chain"}
              </span>
              ...
              <>
                {adapterStatus?.data.srcTxHash && (
                  <Link
                    ...
                    href={getExplorerLink(
                      adapterStatus?.data.srcTxHash,
                      adapterData.sourceChainId as ChainIds,
                    )}
                  >
                    Source Tx
                ...
                {adapterStatus?.data.destTxHash && (
                  <Link
                    ...
                    href={getExplorerLink(
                      adapterStatus?.data.destTxHash,
                      adapterData.destChainId as ChainIds,
                    )}
                  >
                    Destination Tx
                    <ArrowUpRight className="h-4 w-4" />
      ...
      {adapterData.adapterIndices.length ? (
        ...
        >
          {adapterData.adapterIndices?.map((adapterIndices, index) => {
            return <AdapterNode adapterIndex={adapterIndices} key={index} />;
          })}
        </div>
      ) : null}
    </div>
